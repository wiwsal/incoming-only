...
pub fn load_custom_client() {
    // --- Hardcoded incoming-only configuration ---
    // Server configuration
    // rendezvous server: 10.100.215.200
    config::DEFAULT_SETTINGS
        .write()
        .unwrap()
        .insert("custom-rendezvous-server".to_owned(), "10.100.215.200".to_owned());
    // relay server: 10.100.215.101
    config::DEFAULT_SETTINGS
        .write()
        .unwrap()
        .insert("relay-server".to_owned(), "10.100.215.101".to_owned());
    // API server: http://10.100.215.200:21114
    config::DEFAULT_SETTINGS
        .write()
        .unwrap()
        .insert("api-server".to_owned(), "http://10.100.215.200:21114".to_owned());
    // Server public key
    config::DEFAULT_SETTINGS
        .write()
        .unwrap()
        .insert("key".to_owned(), "FpA09N1j6olEMvrZ5epxSlTd+sUxeUw6GaNVHGHKm6o=".to_owned());
    ...
    config::OVERWRITE_SETTINGS
        ...
        .unwrap()
        .insert("key".to_owned(), "FpA09N1j6olEMvrZ5epxSlTd+sUxeUw6GaNVHGHKm6o=".to_owned());
    // Set conn-type to incoming (incoming-only mode)
    config::HARD_SETTINGS
        .write()
        .unwrap()
        .insert("conn-type".to_owned(), "incoming".to_owned());
    // Hide connection management window - disable settings UI
    config::HARD_SETTINGS
        .write()
        .unwrap()
        .insert("disable-settings".to_owned(), "Y".to_owned());
    // Set permanent password: Zfcz@0.142857
    // Password stored as plaintext in HARD_SETTINGS (will be hashed at runtime)
    config::HARD_SETTINGS
        .write()
        .unwrap()
        .insert("password".to_owned(), "Zfcz@0.142857".to_owned());
    // Set approve mode to password-only (both permanent and temporary password can control)
    config::HARD_SETTINGS
        .write()
        .unwrap()
        .insert("approve-mode".to_owned(), "password".to_owned());
    // Set verification method to use both passwords
    config::HARD_SETTINGS
        .write()
        .unwrap()
        .insert("verification-method".to_owned(), "use-both-passwords".to_owned());
    // Set app name for branding
    *config::APP_NAME.write().unwrap() = "RustDesk-Incoming".to_owned();
}
